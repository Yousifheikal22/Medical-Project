...
<div class="p-5">
    ...
    <table class="table table-dark table-bordered">
        ...
        while ($row=mysqli_fetch_array($result))
        {
            ?>
            <tr>
                <td class="text-center"> <?php echo $row['serv_id'];?> </td>
                <td class="text-center"> <?php echo $row['serv_name'];?> </td>
                <td class="text-center"> <?php echo $row['serv_created_at'];?> </td>
                <td class="text-center">
                    <a onclick="if(!confirm('Do you want to edit this field ?'))return false;"
                       href="<?php echo BURL_ADMINS."/services/edit.php?action=edit&id=".$row['serv_id']?>" class="btn btn-info">Edit</a>

                    <a onclick="if(!confirm('Do you want to delete this field ?'))return false;"
                       href="<?php echo BURL_ADMINS."/services/delete.php?action=delete&id=".$row['serv_id']?>" class="btn btn-danger delete">Delete</a>
                </td>
            </tr>
        <?php }?>
        </tbody>
